...
int main(){
    ...
    for(int r = 0; r < SIZE; r++){
        for(int t = 0; t < NUM_THREADS; t++){
            //비효율적이다. 한 스레드가 여러 행을 처리하도록 하는 게 오버헤드를 줄이는 길
            thread_data_arr[t].matrix = matrix;            
            thread_data_arr[t].row = r;
            thread_data_arr[t].start_col = t * 40;
            thread_data_arr[t].end_col = (t + 1) * 40;
    ...
    printf("coarse-grain - inefficient\n");
    printf("UNSALTED : %d\n", check);
